...
function CommentWrite({
  ...
  const onCommentSubmit = async () => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_REQUEST_URL}/api/comment/write`,
      formData
    );
...
function Comment({ userId, articleId }) {
  ...
  const onDeleteClick = async (e) => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_REQUEST_URL}/api/comment/delete`,
      {
        articleId,
        ...
        {commentList.map((data, index) => {
          return (
            <div className="block" key={data.commentIndex}>
              <div
                className={"media " + (data.depth === 0 ? "" : "ml-5")}
              ...
              {clickedComment === data.commentIndex ? (
                <CommentWrite
                  articleId={articleId}
                  depth={1}
                  targetIndex={data.commentIndex}
